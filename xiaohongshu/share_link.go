...
type ShareLinkResult struct {
	...
	CoverURL string `json:"cover_url,omitempty"` // 封面图 URL（图文/视频均有）
...
type noteEntry struct {
	NoteURL  string // /explore/{id}?xsec_token=...
	FeedID   string
	Title    string
	NoteType string // normal / video
	CoverURL string // 封面图，来自 feed 列表，不需要额外请求
}

// GetUserAllShareLinks 获取指定用户主页上所有笔记的分享链接。
// 复用 UserProfileAction 拉取全量 feeds（已处理虚拟列表），
// 再逐个导航到笔记详情页点击分享按钮获取链接。
...
func (a *ShareLinkAction) GetUserAllShareLinks(ctx context.Context, userID, xsecToken string, maxScrollCount int) ([]ShareLinkResult, error) {
	...
	// 1. 复用 UserProfileAction 获取全量 feeds，它通过 __INITIAL_STATE__ 读取，不受虚拟列表影响
	profileAction := NewUserProfileAction(page)
	profile, err := profileAction.UserProfile(ctx, userID, xsecToken, maxScrollCount)
	if err != nil {
		return nil, fmt.Errorf("获取用户主页失败: %w", err)
	}

	feeds := profile.Feeds
	logrus.Infof("share_link: 用户 %s 共 %d 条笔记，开始逐个获取分享链接", userID, len(feeds))

	if len(feeds) == 0 {
		return nil, fmt.Errorf("未找到任何笔记，请确认用户主页已加载")
	}

	// 2. 构造 entries（从 feed 里拼 explore URL，同时拿封面）
	entries := make([]noteEntry, 0, len(feeds))
	for _, f := range feeds {
		if f.ID == "" || f.XsecToken == "" {
			continue
		}
		entries = append(entries, noteEntry{
			NoteURL:  makeFeedDetailURL(f.ID, f.XsecToken),
			FeedID:   f.ID,
	...
	results := make([]ShareLinkResult, 0, len(entries))
	...
	for i, entry := range entries {
		...
		shareURL, noteType, videoURL, err := a.getShareLinkByNoteURL(ctx, page, entry.NoteURL, entry.FeedID)
		if noteType != "" {
			result.NoteType = noteType // 以详情页实际类型为准
		}
		if err != nil {
			logrus.Warnf("share_link: [%d/%d] 笔记「%s」获取失败: %v", i+1, len(entries), entry.Title, err)
		...
		if (i+1)%5 == 0 {
			...
			logrus.Infof("share_link: 已处理 %d/%d，短暂休息 %v", i+1, len(entries), extra)
			time.Sleep(extra)
		}
	}
...
// getShareLinkByNoteURL 直接导航到笔记详情页，注入 clipboard 拦截，点击分享按钮，同时提取视频 URL。
// explore 路径 token 过期时自动回退到 discovery/item 路径重试。
func (a *ShareLinkAction) getShareLinkByNoteURL(ctx context.Context, page *rod.Page, noteURL, feedID string) (shareURL, noteType, videoURL string, err error) {
	logrus.Infof("share_link: 打开笔记 %s", feedID)
	page.MustNavigate(noteURL)
	page.MustWaitDOMStable()
	time.Sleep(time.Duration(500+rand.Intn(300)) * time.Millisecond)

	// explore 路径 token 过期时自动回退到 discovery/item 路径
	curr := page.MustInfo().URL
	if !isNoteDetailPage(curr) {
		if feedID == "" {
			return "", "", "", fmt.Errorf("未到达详情页，当前 URL: %s", curr)
		}
		logrus.Warnf("share_link: explore 路径 404，回退到 discovery/item 路径 (feedID=%s)", feedID)
		fallbackURL := fmt.Sprintf(
			"https://www.xiaohongshu.com/discovery/item/%s?xsec_source=pc_share",
			feedID,
		)
		...
		curr = page.MustInfo().URL
		if !isNoteDetailPage(curr) {
			return "", "", "", fmt.Errorf("回退路径仍未到达详情页，当前 URL: %s", curr)
		}
	}
	...
	if findErr != nil {
		return "", noteType, videoURL, fmt.Errorf("未找到分享按钮: %w", findErr)
	}
	...
	if err := shareBtn.Click(proto.InputMouseButtonLeft, 1); err != nil {
		return "", noteType, videoURL, fmt.Errorf("点击分享按钮失败: %w", err)
	}
	...
	if captured == "" {
		return "", noteType, videoURL, fmt.Errorf("clipboard 未捕获到内容")
	}

	shareURL = extractShareURL(captured)
	if shareURL == "" {
		return "", noteType, videoURL, fmt.Errorf("clipboard 内容中未找到有效 URL")
	}

	return shareURL, noteType, videoURL, nil
}
